...
function AuthenticatedApp({ session, onboardingJustCompleted = false }) {
  ...
  const { transactions, loading: txLoading, refreshing: txRefreshing, createTransaction, deleteTransaction, updateTransaction, autoRefetchTransactions, refreshTransactions } = useTransactions(session.user.id, limits, { syncBalances });
  ...
  const isBasicAtMax = !subscription.isPro && customCategories.filter(c => !c.is_deleted).length >= (limits?.maxCustomCategories ?? Infinity);
  // Wrapper: deleteCustomCategory sekarang butuh isPro sebagai guard
