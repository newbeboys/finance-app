...
// Step "Saring per Dompet" hanya ada kalau user punya lebih dari 1 dompet
// (dropdown-nya sendiri memang tak dirender di DOM kalau cuma 1 dompet).
// Teks step "Unduh Laporan" beda buat Basic vs Pro supaya tak bikin user
// Basic mengira fitur ini gratis.
function buildSteps(hasMultipleWallets, isPro) {
  const steps = [
    { targetIds: ['analitik-scope-toggle'], titleKey: 'tour.analitik.gantiRentang.title', descKey: 'tour.analitik.gantiRentang.desc' },
  ];
  if (hasMultipleWallets) {
    steps.push({ targetIds: ['analitik-wallet-filter'], titleKey: 'tour.analitik.saringDompet.title', descKey: 'tour.analitik.saringDompet.desc' });
  }
  steps.push({
    targetIds: ['analitik-unduh-laporan'],
    titleKey: 'tour.analitik.unduhLaporan.title',
    descKey: isPro ? 'tour.analitik.unduhLaporan.descPro' : 'tour.analitik.unduhLaporan.descBasic',
  });
  return steps;
}
...
export default function AnalitikTour({ isActive, onComplete, hasMultipleWallets = false, isPro = false }) {
  const { t } = useTranslation();
  const STEPS = React.useMemo(() => buildSteps(hasMultipleWallets, isPro), [hasMultipleWallets, isPro]);
