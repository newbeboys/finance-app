...
// Step 2 (tambah dana ke goal pertama) hanya ada kalau user sudah punya
// minimal 1 goal — kalau list kosong, tour cukup 1 step lalu langsung selesai.
function buildSteps(hasGoals) {
  const steps = [
    { targetIds: ['goal-add'], titleKey: 'tour.tabungan.buatGoal.title', descKey: 'tour.tabungan.buatGoal.desc' },
  ];
  if (hasGoals) {
    steps.push({ targetIds: ['goal-deposit-first'], titleKey: 'tour.tabungan.tambahDana.title', descKey: 'tour.tabungan.tambahDana.desc' });
  }
  return steps;
}
...
export default function SavingsTour({ isActive, onComplete, hasGoals = false }) {
  const { t } = useTranslation();
  const STEPS = React.useMemo(() => buildSteps(hasGoals), [hasGoals]);
