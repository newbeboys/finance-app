...
function buildSteps(isPro, hasMultipleWallets) {
  // Step 2 menyorot union area filter — kalau dropdown "Semua Dompet" tampil
  // (accounts.length > 1), cutout-nya diperluas mencakup dropdown itu juga
  // (getUnionRect di bawah menggabungkan bounding rect semua targetIds jadi
  // satu), supaya tak ada elemen filter yang lolos tak ter-highlight.
  const searchFilterIds = hasMultipleWallets ? ['tx-wallet-filter', 'tx-search-filter'] : ['tx-search-filter'];
  return [
    {
      targetIds: isPro ? ['tx-scan', 'tx-add'] : ['tx-add'],
      titleKey: isPro ? 'tour.transaksi.catatScan.title' : 'tour.transaksi.catatManual.title',
      descKey: isPro ? 'tour.transaksi.catatScan.desc' : 'tour.transaksi.catatManual.desc',
    },
    {
      targetIds: searchFilterIds,
      titleKey: 'tour.transaksi.cariSaring.title',
      descKey: hasMultipleWallets ? 'tour.transaksi.cariSaring.descDompet' : 'tour.transaksi.cariSaring.desc',
    },
  ];
}

// Gabungkan bounding rect dari beberapa elemen target jadi satu rect (union),
// dipakai saat sebuah step perlu menyorot lebih dari satu elemen sekaligus.
...
export default function TransaksiTour({ isActive, onComplete, isPro = false, hasMultipleWallets = false }) {
  const { t } = useTranslation();
  const STEPS = React.useMemo(() => buildSteps(isPro, hasMultipleWallets), [isPro, hasMultipleWallets]);
  ...
  return (
    <>
      ...
      {/* Spotlight: cutout terang di sekitar target (union rect kalau lebih dari satu elemen),
          pakai trik box-shadow (tanpa lib eksternal) */}
      {rect && (
        <div
          style={{
            position: 'fixed',
