...
// Step 2 (Pratinjau + Unduh) hanya ada kalau ada minimal 1 kartu laporan —
// kalau belum ada transaksi sama sekali, tak ada kartu buat disorot, jadi
// tour cukup 1 step lalu langsung selesai (sama pola seperti tour Tabungan).
//
// Teks step 2 TIDAK pakai percabangan title/desc penuh seperti tour Analitik
// (Scan/Unduh Laporan), karena hasil cek gating (reports.jsx:1064-1066)
// menunjukkan hanya tombol "Unduh" yang Pro-only — "Pratinjau" gratis untuk
// semua tier. Jadi teksnya netral, dan Basic cuma dapat catatan kecil
// tambahan soal Unduh, bukan teks yang sepenuhnya berbeda.
function buildSteps(hasReports, canExport) {
  const steps = [
    { targetIds: ['laporan-scope-toggle'], titleKey: 'tour.laporan.pilihJenis.title', descKey: 'tour.laporan.pilihJenis.desc' },
  ];
  if (hasReports) {
    steps.push({
      targetIds: ['laporan-aksi'],
      titleKey: 'tour.laporan.lihatUnduh.title',
      descKey: canExport ? 'tour.laporan.lihatUnduh.desc' : 'tour.laporan.lihatUnduh.descBasic',
    });
  }
  return steps;
}
...
export default function LaporanTour({ isActive, onComplete, hasReports = false, canExport = false }) {
  const { t } = useTranslation();
  const STEPS = React.useMemo(() => buildSteps(hasReports, canExport), [hasReports, canExport]);
