...
const STEPS = [
  { targetId: null,           titleKey: 'tour.welcome.title',   descKey: 'tour.welcome.desc' },
  { targetId: 'bottom-nav',   titleKey: 'tour.bottomNav.title', descKey: 'tour.bottomNav.desc' },
];
...
export default function ProductTour({ isActive, onComplete }) {
  ...
  // belum ter-mount saat step berganti. Step tanpa targetId (mis. sambutan) tidak
  // punya spotlight, jadi tak perlu dicari elemennya.
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Spotlight: cutout terang di sekitar target, pakai trik box-shadow (tanpa lib eksternal).
          Hanya tampil untuk step yang punya target (bukan step sambutan). */}
      {hasTarget && rect && (
        <div
          style={{
            position: 'fixed',
