...
There is no Redux/Zustand/Context-based data store. Each domain has a dedicated hook in `src/hooks/` (`useTransactions`, `useSavings`, `useWallets`, `useWalletMembers`, `useBudgets`, `useDebts`, `useCustomCategories`, `useSubscription`, `useNotifications`, `useRevenueCat`) that calls the Supabase client (`src/supabase.js`) directly, keeps its own `useState`, and is composed together in `app.jsx`. Every query is scoped client-side *and* by Postgres RLS server-side (defense in depth) — always constrain the query explicitly even though RLS would also block cross-user access. For most domains that constraint is `.eq('user_id', userId)`. **Exception: `wallets` and `transactions`**, which since the shared-wallet feature are scoped by "own rows **or** a wallet you're an active member of" — those two use `sharedOrFilter()` from `src/lib/walletAccess.js` instead (it falls back to a plain `.eq('user_id', …)` when the user has no shared wallets). Don't "fix" them back to a bare `user_id` filter; that silently hides shared-wallet data. The `wallet_id.in.(…)` id list fed into `sharedOrFilter()` differs by hook: `useWallets` only needs `fetchSharedMemberships()` (dompet dibagikan KE me — my own wallets are already covered by the bare `user_id.eq.me` branch, since a wallet row is always authored by its owner), but `useTransactions` also needs `fetchOwnedWalletIds()` merged in (dompet yang saya MILIKI) — a transaction row in my own wallet can be authored by someone else's `user_id` once shared wallets exist, so `user_id.eq.me` alone misses it. Omitting `fetchOwnedWalletIds()` from `useTransactions` was a real bug (found+fixed 13 Sep 2026): the wallet owner could never see a member's transactions, in any fetch, ever — RLS already allowed it, the client just never asked. Regression tests: `tests/owner-visibility.harness.mjs` and `tests/shared-wallet-sync.spec.mjs`. Consequence: those arrays contain rows the user may **not** write. An UPDATE/DELETE that RLS rejects returns **0 rows, not an error** — so direct writes on `wallets` must `.select('id')` and treat an empty result as failure, and UI must gate with `canEditTransaction()`/`canDeleteTransaction()` and the `isShared`/`canWrite` flags from `useWallets` (use `writableAccounts` for any picker that records money). Since 12 Sep 2026 viewers and ex-members cannot update or delete *any* transaction in that wallet — not even rows they recorded themselves — so both gates require the wallet's `canWrite` (see decision #6 in `teknis_arsitektur-database.md`). **The edit gate and the delete gate stopped being the same function on 13 Sep 2026 (decision #7, migration `20260919000000`) — never collapse them back.** `canEditTransaction()` delegates to the strict `canDeleteOwnTransaction()` (own row only); `canDeleteTransaction()` is deliberately looser — own row **or** `wallet.role === 'owner'` — so an owner can clean up a member's row, including rows whose author has already left the wallet (previously undeletable by anyone). Owners still cannot *edit* those rows. `useDebts.deleteDebt` is the one caller deliberately kept on the strict gate: the RPC's owner branch requires `debt_id IS NULL`, so a looser pre-flight would promise a delete the server then refuses. `canDeleteTransaction()` itself does **not** check `debt_id` — those guards live elsewhere (the `excludeDebt` fetch filter keeps other people's debt rows out of state, and the RPC enforces it server-side). Transaction create/update/delete go **only** through the atomic RPCs `record_transaction`/`update_transaction`/`delete_transaction`, which write the row and the wallet balance together; the client never adjusts balances separately (`adjustBalance()` was removed — a "succeeded" 0-row delete followed by a separate balance adjustment corrupted balances once). New balances reach React state through **two writers, both absolute, never delta**: `useWallets.syncBalances()`, which `useTransactions` calls after every successful write RPC, and the `wallets_lock` realtime channel for changes made elsewhere. Realtime used to be the *only* writer — and it was silently dead in production until 14 Sep 2026 (same channel as a `wallet_members` binding, see "Realtime publication" below) — so don't go back to relying on realtime alone for money shown on screen. Debt-linked transactions (`debt_id` set) are never shared: only their recorder can see them, even in a shared wallet. `useTransactions` has **no realtime subscription** — shared-wallet changes arrive via a foreground refetch (`autoRefetchTransactions()`, hung off the existing `visibilitychange`/`focus` listener in `app.jsx`). **This is Phase 1**, not the final state: realtime on `transactions` is planned as Phase 2, and the manual Segarkan button stays as a safety net even after Phase 2 lands — don't remove it. Both fetch paths **queue** behind in-flight writes via `drainWrites()` (`while` + `Promise.allSettled(inFlightWritesRef)`) — never skip: a fetch that leaves before the write RPC commits would roll the list back to its pre-write state. Only the auto path has cancelling brakes, and it silently skips (never queues) on them: while the PIN/biometric gate is up (`isAppLocked()` in `hooks/useAutoLock.js` — a module-level flag, since React state is not yet committed inside the event handler that triggers the lock), or within 60 s of the last successful fetch. Both brakes are re-checked **after** the queue wait, since the app can lock or another fetch can land while we wait. The manual Segarkan button on the Transactions page (`refreshTransactions()`) ignores both brakes.
...
`src/utils/dateLocal.js` (`dateToISO(d)`, `todayISO()`, `monthPrefixISO(d)` → "YYYY-MM") adalah sumber TUNGGAL untuk mengubah Date menjadi string lokal "YYYY-MM-DD"/"YYYY-MM". Konsolidasi LENGKAP mencakup: `transactions.jsx`, `debts-page.jsx`, `widgets.jsx`, `components/debts/AddDebtModal.jsx`, `components/debts/DebtDetailSheet.jsx`, `components/RecurringTransactionForm.jsx`, `hooks/useDebts.js`, `hooks/useNotifications.js` (batch 1), plus `hooks/useTransactions.js` dan `lib/recurringHelper.js` (batch 2, highest-risk). Batch 0 timezone menambah: `lib/debtProof.js` (`todayISO()`), `toISO` di `recurringHelper.js` (kini alias `dateToISO`; `pad` lokal dihapus), dan `monthPrefixISO` di `analytics.jsx`, `widgets.jsx`, `hooks/useNotifications.js`, `lib/budgetSpent.js`. Perilaku identik byte-per-byte, hanya shape yang berubah. `recurringHelper.js` masih menyimpan `fromISO` lokal (dipakai `RecurringTransactionForm.jsx`). `toISO` di sana di-**import sebagai binding lokal lalu di-export ulang** (`export { toISO }`) — jangan diganti jadi `export { dateToISO as toISO } from '…'`, karena re-export murni tidak membuat binding lokal dan 4 pemanggil internalnya jadi `ReferenceError` (build tetap lolos). Belum ikut: inline di `lib/widgetSync.js` dan pola `${yr}-${…mo + 1…}` (bukan `X.getFullYear()`) di `analytics.jsx`/`widgets.jsx`. `toISOString()` yang tersisa di `useSubscription.js:97`/`useDebts.js:250` SENGAJA — dibandingkan dengan kolom timestamptz (instant penuh), bukan tanggal kalender.
...
Tables (all RLS-protected): `transactions`, `savings`, `budgets`, `wallets`, `debts`, `debt_payments`, `custom_categories`, `user_subscriptions`, `error_logs`, `wallet_members`. Most are scoped by `user_id`; `wallets` and `transactions` are scoped by "owner **or** active member" — see below.

**Realtime publication (updated 15 Sep 2026 via `pg_publication_tables`):** `supabase_realtime` now contains 8 tables: `budgets`, `custom_categories`, `debts`, `savings`, `transactions`, `user_subscriptions`, `wallet_members`, `wallets`. `wallet_members` and `debts` were added 15 Sep 2026 via `ALTER PUBLICATION ADD TABLE` (fixing the dead `wallets_lock`/`wallet_members_sheet` and `debts:<uid>` channels — see the now-superseded 14 Sep 2026 note this replaced). `debt_payments` is deliberately excluded — `useDebts` has no binding on it. A `postgres_changes` binding on a table outside the publication kills the **whole channel**, valid bindings included — so keep one table per channel, and give channels re-created by a re-running effect a unique topic suffix (`supabase.channel(topic)` returns the existing instance while an async `removeChannel` is still pending). Base schema in `supabase/schema.sql`, `supabase/custom_categories.sql`, `supabase/subscriptions.sql`; incremental changes in `supabase/migrations/*.sql` (timestamp-prefixed, applied in order via `supabase db push`).
