...
On the client, `src/utils/dateLocal.js` (`dateToISO(d)`, `todayISO()`) is the **single source** for turning a `Date` into a local `YYYY-MM-DD` — the same getFullYear/getMonth/getDate + padStart body had been copy-pasted into eight places. Consolidated (behaviour-identical, no bug fixed) in `transactions.jsx`, `debts-page.jsx`, `widgets.jsx`, `components/debts/AddDebtModal.jsx`, `components/debts/DebtDetailSheet.jsx`, `components/RecurringTransactionForm.jsx`, `hooks/useDebts.js`, and `hooks/useNotifications.js` (where the local name `localISO` is kept as an alias so its call sites did not change). Import it instead of re-deriving the format. **Still deliberately outside this consolidation:** `src/lib/recurringHelper.js` keeps its own `toISO`/`todayISO` (identical bodies) — it drives recurring-transaction *execution*, so it is being folded in as a separate batch; the inline copies in `hooks/useTransactions.js` and `lib/widgetSync.js` are likewise not migrated yet. `src/lib/debtProof.js` is **not** a consolidation target: its "generated on" stamp uses `toISOString()` (UTC), so swapping in `todayISO()` would change output, not just shape — see the open finding, not a silent refactor.
