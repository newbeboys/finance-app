...
`src/utils/dateLocal.js` (`dateToISO(d)`, `todayISO()`) adalah sumber TUNGGAL untuk mengubah Date menjadi string lokal "YYYY-MM-DD". Konsolidasi LENGKAP mencakup: `transactions.jsx`, `debts-page.jsx`, `widgets.jsx`, `components/debts/AddDebtModal.jsx`, `components/debts/DebtDetailSheet.jsx`, `components/RecurringTransactionForm.jsx`, `hooks/useDebts.js`, `hooks/useNotifications.js` (batch 1), plus `hooks/useTransactions.js` dan `lib/recurringHelper.js` (batch 2, highest-risk). Perilaku identik byte-per-byte, hanya shape yang berubah. `recurringHelper.js` tetap menyimpan `toISO`/`fromISO`/`pad` lokal (hanya `todayISO` yang di-import); `fromISO` masih dipakai `RecurringTransactionForm.jsx`.
