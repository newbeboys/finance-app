...
**`useTransactions` juga butuh `fetchOwnedWalletIds` (bug ditemukan+diperbaiki 13 Sep 2026, lihat catatan Q5).** `fetchSharedWalletIds` sengaja TIDAK memuat dompet milik sendiri, dengan alasan "pemanggil sudah menyaringnya lewat `user_id`" — itu benar untuk tabel `wallets` (baris dompet selalu ditulis pemiliknya sendiri) tapi SALAH untuk `transactions`, karena baris transaksi di dompet SAYA bisa ditulis ORANG LAIN sejak dompet bersama ada. `useTransactions` menggabung `fetchSharedWalletIds` (dompet yang dibagikan ke saya) DAN `fetchOwnedWalletIds` (dompet yang saya miliki) jadi satu daftar `wallet_id.in.(…)`, supaya cabang itu mencerminkan persis policy SELECT server (`wallet_access_role(wallet_id) IS NOT NULL`). `useWallets` TIDAK diubah — di sana asumsi lamanya tetap benar.
